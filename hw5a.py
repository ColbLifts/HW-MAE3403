...
def ff(Re, rr, CBEQN=False):
    ...
    if(CBEQN):
        ...
        cb=lambda f:1.0/np.sqrt(f)+2.0*np.log10(rr/3.7 + 2.51/(Re*np.sqrt(f))) #Equation for friction factor using Colebrook.
        result=fsolve(cb, 0.02) # Using fsolve and the colebrook to solve for ff.
        return result[0]
    else:
        return 64/Re #Equation for laminar flow
    pass
...
def plotMoody(plotPoint=False, pt=(0,0)):
    ...
    plt.ylabel(r"Friction factor $f=\frac{h_f}{\left(\frac{L}{D}\frac{V^2}{2g}\right)}$", fontsize=16)
    ...
    plt.grid(which='both')
    if(plotPoint):
        plt.plot(pt[0], pt[1], marker='^' if 2000< pt[0] < 4000 else 'o', markersize=12, markeredgecolor='red', markerfacecolor='none')
