...
Assumptions:
	While the actual gravel is not spherical, we will assume that the rocks are spherical.
...
def F_tlnpdf(args):
    ...
    # Replacement of Simpson with quad function
...
def makeSample(args, N=100):
    ...
    # Replacement of Secant with fsolve
    # fsolve returns an array, so we take the first element [0]
    d_s = [fsolve(lambda D: F_tlnpdf((ln_Mean, ln_sig, D_Min, D_Max, D[0], F_DMax, F_DMin)) - probs[i],
                  (D_Max + D_Min) / 2)[0] for i in range(len(probs))]
    return d_s
...
def getFDMaxFDMin(args):
    ...
    # Corrected quad calls to only pass required (D, mu, sig) tuple to ln_PDF
    F_DMax, _ = quad(lambda x: ln_PDF((x, mean_ln, sig_ln)), 0, D_Max)
    F_DMin, _ = quad(lambda x: ln_PDF((x, mean_ln, sig_ln)), 0, D_Min)
    return (F_DMin, F_DMax)
#F_DMax and Min are now changed to accept quad call.
...
def main():
    '''
    Assumptions: Gravel rocks are assumed to be spherical.
    Initial distribution is log-normal; sieved product is a truncated log-normal distribution.

    This program simulates a gravel production process.
    Step 1: Get user input for mean of ln(D), stdev of ln(D), Dmax, Dmin, N_samples, N_sampleSize
    Step 2: Produce random samples from the truncated distribution using SciPy quad and fsolve.
    Step 3: Compute and report sample mean and variance.
    ...
    while (goAgain == True):
        ...
        Samples, Means = makeSamples(
            (mean_ln, sig_ln, D_Min, D_Max, F_DMax, F_DMin, int(N_sampleSize), int(N_samples), True))
